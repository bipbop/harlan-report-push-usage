	...
	harlan.addPlugin(function (controller) {
	  function drawReport(data) {
	    ...
	    var chart = new Chart(chartCanvas.getContext('2d'), {
	      ...
	      data: {
	        labels: map_1(data, function (_ref) {
	          var username = _ref.username;
	          return username;
	        }),
	        datasets: [{
	          label: 'Crescimento de PUSH',
	          data: map_1(data, function (_ref2) {
	            var total = _ref2.total;
	            return total;
	          }),
	          backgroundColor: map_1(data, function () {
	            return colors.shift();
	          })
	        }]
	      }
	  ...
	  controller.registerTrigger('call::authentication::loggedin', '', function (args, callback) {
	    callback();
	    controller.server.call("SELECT FROM 'BIPBOPCOMPANYSREPORT'.'PUSHUSAGE'", {
	      dataType: 'json',
	      success: function success(data) {
	        return drawReport(data);
	      }
	    });
	  });
	});
